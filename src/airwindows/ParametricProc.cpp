...
void Parametric::processReplacing(float** inputs, float** outputs, int sampleFrames)
{
    float* in1 = inputs[0];
    float* in2 = inputs[1];
    ...
    high[biqs_reso] = ((0.5 + (high[biqs_nonlin] * 0.5) + sqrt(high[biqs_freq])) - (1.0 - pow(1.0 - C, 2.0))) + 0.5 +
                      (high[biqs_nonlin] * 0.5);
    ...
    hmid[biqs_reso] = ((0.5 + (hmid[biqs_nonlin] * 0.5) + sqrt(hmid[biqs_freq])) - (1.0 - pow(1.0 - F, 2.0))) + 0.5 +
                      (hmid[biqs_nonlin] * 0.5);
    ...
    lmid[biqs_reso] = ((0.5 + (lmid[biqs_nonlin] * 0.5) + sqrt(lmid[biqs_freq])) - (1.0 - pow(1.0 - I, 2.0))) + 0.5 +
                      (lmid[biqs_nonlin] * 0.5);
...
void Parametric::processDoubleReplacing(double** inputs, double** outputs, int sampleFrames)
{
    double* in1 = inputs[0];
    double* in2 = inputs[1];

...
void Channel9::processReplacing(float** inputs, float** outputs, int sampleFrames)
{
    float* in1 = inputs[0];
    float* in2 = inputs[1];
    ...
    biquadB[0] = biquadA[0] = cutoff / sampleRate;
    ...
    while (--sampleFrames >= 0)
    {
        ...
        {
            tempSample = biquadA[2] * inputSampleL + biquadA[3] * biquadA[7] + biquadA[4] * biquadA[8] -
            ...
            tempSample = biquadA[2] * inputSampleR + biquadA[3] * biquadA[11] + biquadA[4] * biquadA[12] -
        ...
        if (flip)
        {
            ...
            iirSampleLA = (iirSampleLA * (1.0 - (localiirAmount * dielectricScaleL))) +
            ...
            iirSampleRA = (iirSampleRA * (1.0 - (localiirAmount * dielectricScaleR))) +
        ...
        else
        {
            ...
            iirSampleLB = (iirSampleLB * (1.0 - (localiirAmount * dielectricScaleL))) +
                          (inputSampleL * localiirAmount * dielectricScaleL);
            inputSampleL = inputSampleL - iirSampleLB;
            if (fabs(iirSampleRB) < 1.18e-37) iirSampleRB = 0.0;
            iirSampleRB = (iirSampleRB * (1.0 - (localiirAmount * dielectricScaleR))) +
                          (inputSampleR * localiirAmount * dielectricScaleR);
        ...
        if (clamp > localthreshold) inputSampleL = lastSampleBL + localthreshold;
        if (-clamp > localthreshold) inputSampleL = lastSampleBL - localthreshold;

        lastSampleAL =
            (lastSampleAL * 0.381966011250105) + (inputSampleL * 0.6180339887498948482045);  // split the difference between
                                                                                             // raw and smoothed for buffer
        // end L
        ...
        if (clamp > localthreshold) inputSampleR = lastSampleBR + localthreshold;
        if (-clamp > localthreshold) inputSampleR = lastSampleBR - localthreshold;

        lastSampleAR =
            (lastSampleAR * 0.381966011250105) + (inputSampleR * 0.6180339887498948482045);  // split the difference between
                                                                                             // raw and smoothed for buffer
        // end R
        ...
        {
            tempSample = biquadB[2] * inputSampleL + biquadB[3] * biquadB[7] + biquadB[4] * biquadB[8] -
            ...
            tempSample = biquadB[2] * inputSampleR + biquadB[3] * biquadB[11] + biquadB[4] * biquadB[12] -
                         biquadB[5] * biquadB[13] - biquadB[6] * biquadB[14];
...
void Channel9::processDoubleReplacing(double** inputs, double** outputs, int sampleFrames)
{
    double* in1 = inputs[0];
    double* in2 = inputs[1];

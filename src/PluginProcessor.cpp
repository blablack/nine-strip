...
NineStripProcessor::NineStripProcessor()
  ...
  addParameter(channel9_consoleType = new juce::AudioParameterChoice(
                   ...
                   juce::StringArray{"Neve", "API", "SSL", "Teac", "Mackie"},
                   0));

  addParameter(
      channel9_drive = new juce::AudioParameterFloat(
  ...
  addParameter(channel9_output = new juce::AudioParameterFloat(
                   "output", "Output", 0.0f, 1.0f, 1.0f));

  // Lowpass2 parameters
  addParameter(lowpass2_lowpass = new juce::AudioParameterFloat(
                   "lowpass", "Lowpass", 0.0f, 1.0f, 0.0f));
  addParameter(lowpass2_sft_hrd = new juce::AudioParameterFloat(
                   "sft_hrd", "Soft/Hard", 0.0f, 1.0f, 0.5f));
  addParameter(lowpass2_poles = new juce::AudioParameterFloat(
                   "poles", "Poles", 0.0f, 1.0f, 0.25f));
  addParameter(lowpass2_dry_wet = new juce::AudioParameterFloat(
                   "dry_wet", "Dry/Wet", 0.0f, 1.0f, 1.0f));

  // Highpass2 parameters
  addParameter(highpass2_hipass = new juce::AudioParameterFloat(
                   "hipass", "Hipass", 0.0f, 1.0f, 0.0f));
  addParameter(highpass2_ls_tite = new juce::AudioParameterFloat(
                   "ls_tite", "Ls/Tite", 0.0f, 1.0f, 0.5f));
  addParameter(highpass2_poles = new juce::AudioParameterFloat(
                   "hp_poles", "HP Poles", 0.0f, 1.0f, 0.25f));
  addParameter(highpass2_dry_wet = new juce::AudioParameterFloat(
                   "hp_dry_wet", "HP Dry/Wet", 0.0f, 1.0f, 1.0f));

  // Baxandall2 parameters
  addParameter(baxandall2_treble = new juce::AudioParameterFloat(
                   "treble", "Treble", 0.0f, 1.0f, 0.5f));
  addParameter(baxandall2_bass = new juce::AudioParameterFloat(
                   "bass", "Bass", 0.0f, 1.0f, 0.5f));

  // Parametric parameters
  addParameter(parametric_tr_freq = new juce::AudioParameterFloat(
                   "tr_freq", "Tr Freq", 0.0f, 1.0f, 0.5f));
  addParameter(parametric_treble = new juce::AudioParameterFloat(
                   "treble_param", "Treble", 0.0f, 1.0f, 0.5f));
  addParameter(parametric_tr_reso = new juce::AudioParameterFloat(
                   "tr_reso", "Tr Reso", 0.0f, 1.0f, 0.5f));
  addParameter(parametric_hm_freq = new juce::AudioParameterFloat(
                   "hm_freq", "HM Freq", 0.0f, 1.0f, 0.5f));
  addParameter(parametric_highmid = new juce::AudioParameterFloat(
                   "highmid", "HighMid", 0.0f, 1.0f, 0.5f));
  addParameter(parametric_hm_reso = new juce::AudioParameterFloat(
                   "hm_reso", "HM Reso", 0.0f, 1.0f, 0.5f));
  addParameter(parametric_lm_freq = new juce::AudioParameterFloat(
                   "lm_freq", "LM Freq", 0.0f, 1.0f, 0.5f));
  addParameter(parametric_lowmid = new juce::AudioParameterFloat(
                   "lowmid", "LowMid", 0.0f, 1.0f, 0.5f));
  addParameter(parametric_lm_reso = new juce::AudioParameterFloat(
                   "lm_reso", "LM Reso", 0.0f, 1.0f, 0.5f));
  addParameter(parametric_dry_wet = new juce::AudioParameterFloat(
                   "param_dry_wet", "Param Dry/Wet", 0.0f, 1.0f, 1.0f));

  // Pressure4 parameters
  addParameter(pressure4_pressure = new juce::AudioParameterFloat(
                   "pressure", "Pressure", 0.0f, 1.0f, 0.0f));
  addParameter(pressure4_speed = new juce::AudioParameterFloat(
                   "speed", "Speed", 0.0f, 1.0f, 0.2f));
  addParameter(pressure4_mewiness = new juce::AudioParameterFloat(
                   "mewiness", "Mewiness", 0.0f, 1.0f, 0.5f));
  addParameter(pressure4_output_gain = new juce::AudioParameterFloat(
                   "output_gain", "Output Gain", 0.0f, 1.0f, 1.0f));
}

NineStripProcessor::~NineStripProcessor() {}

//==============================================================================
const juce::String NineStripProcessor::getName() const {
...
void NineStripProcessor::changeProgramName(int index,
...
bool NineStripProcessor::isBusesLayoutSupported(
    const BusesLayout &layouts) const {
  // Only support stereo
  return layouts.getMainOutputChannelSet() == juce::AudioChannelSet::stereo() &&
         layouts.getMainInputChannelSet() == juce::AudioChannelSet::stereo();
}
void NineStripProcessor::processBlock(juce::AudioBuffer<float> &buffer,
...
juce::AudioProcessorEditor *NineStripProcessor::createEditor() {
  return new NineStripProcessorEditor(*this);
}
...
void NineStripProcessor::getStateInformation(juce::MemoryBlock &destData) {
  ...
  std::unique_ptr<juce::XmlElement> xml(
      new juce::XmlElement("NineStripSettings"));
  ...
  xml->setAttribute("sft_hrd", static_cast<double>(lowpass2_sft_hrd->get()));
  xml->setAttribute("poles", static_cast<double>(lowpass2_poles->get()));
  xml->setAttribute("dry_wet", static_cast<double>(lowpass2_dry_wet->get()));
  ...
  xml->setAttribute("hp_dry_wet",
                    static_cast<double>(highpass2_dry_wet->get()));
  ...
  xml->setAttribute("treble_param",
                    static_cast<double>(parametric_treble->get()));
  ...
  xml->setAttribute("param_dry_wet",
                    static_cast<double>(parametric_dry_wet->get()));
  ...
  xml->setAttribute("output_gain",
                    static_cast<double>(pressure4_output_gain->get()));

  copyXmlToBinary(*xml, destData);
}

void NineStripProcessor::setStateInformation(const void *data,
                                             int sizeInBytes) {
  ...
  if (xml.get() != nullptr && xml->hasTagName("NineStripSettings")) {
    ...
    *channel9_output =
        static_cast<float>(xml->getDoubleAttribute("output", 1.0));

    *lowpass2_lowpass =
        static_cast<float>(xml->getDoubleAttribute("lowpass", 0.0));
    *lowpass2_sft_hrd =
        static_cast<float>(xml->getDoubleAttribute("sft_hrd", 0.5));
    *lowpass2_poles =
        static_cast<float>(xml->getDoubleAttribute("poles", 0.25));
    *lowpass2_dry_wet =
        static_cast<float>(xml->getDoubleAttribute("dry_wet", 1.0));

    *highpass2_hipass =
        static_cast<float>(xml->getDoubleAttribute("hipass", 0.0));
    *highpass2_ls_tite =
        static_cast<float>(xml->getDoubleAttribute("ls_tite", 0.5));
    *highpass2_poles =
        static_cast<float>(xml->getDoubleAttribute("hp_poles", 0.25));
    *highpass2_dry_wet =
        static_cast<float>(xml->getDoubleAttribute("hp_dry_wet", 1.0));

    *baxandall2_treble =
        static_cast<float>(xml->getDoubleAttribute("treble", 0.5));
    *baxandall2_bass = static_cast<float>(xml->getDoubleAttribute("bass", 0.5));

    *parametric_tr_freq =
        static_cast<float>(xml->getDoubleAttribute("tr_freq", 0.5));
    *parametric_treble =
        static_cast<float>(xml->getDoubleAttribute("treble_param", 0.5));
    *parametric_tr_reso =
        static_cast<float>(xml->getDoubleAttribute("tr_reso", 0.5));
    *parametric_hm_freq =
        static_cast<float>(xml->getDoubleAttribute("hm_freq", 0.5));
    *parametric_highmid =
        static_cast<float>(xml->getDoubleAttribute("highmid", 0.5));
    *parametric_hm_reso =
        static_cast<float>(xml->getDoubleAttribute("hm_reso", 0.5));
    *parametric_lm_freq =
        static_cast<float>(xml->getDoubleAttribute("lm_freq", 0.5));
    *parametric_lowmid =
        static_cast<float>(xml->getDoubleAttribute("lowmid", 0.5));
    *parametric_lm_reso =
        static_cast<float>(xml->getDoubleAttribute("lm_reso", 0.5));
    *parametric_dry_wet =
        static_cast<float>(xml->getDoubleAttribute("param_dry_wet", 1.0));

    *pressure4_pressure =
        static_cast<float>(xml->getDoubleAttribute("pressure", 0.0));
    *pressure4_speed =
        static_cast<float>(xml->getDoubleAttribute("speed", 0.2));
    *pressure4_mewiness =
        static_cast<float>(xml->getDoubleAttribute("mewiness", 0.5));
    *pressure4_output_gain =
        static_cast<float>(xml->getDoubleAttribute("output_gain", 1.0));
  }
}

juce::AudioProcessor *JUCE_CALLTYPE createPluginFilter() {
  return new NineStripProcessor();
}
